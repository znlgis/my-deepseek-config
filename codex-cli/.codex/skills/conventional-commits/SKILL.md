...
BREAKING CHANGE: the legacy route is removed; callers must
use the new tiers instead.
```

## When NOT to over-engineer

For a tiny one-file tweak, a single-line `type: subject` is enough. Match the
repository's existing commit style if it already follows a different but
...
If the repo uses a different convention, follow the existing style.
