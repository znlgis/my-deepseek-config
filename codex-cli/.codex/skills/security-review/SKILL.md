...
to the **diff** first, then widen only if a finding points elsewhere. Report
findings; do not rewrite code unless explicitly asked.
...
- **AuthN / AuthZ** — missing authentication, missing ownership/permission
  checks, IDOR, privilege escalation, trusting client-supplied roles.
- **Secrets** — hardcoded API keys, tokens, passwords, private keys, or
  connection strings; secrets logged or committed.
...
- **Crypto** — weak algorithms (MD5/SHA1 for passwords), hardcoded IVs,
